...
var databasePath = Path.Combine(AppContext.BaseDirectory, "lab04-start.db");
...
public sealed class OrdersContext(string databasePath) : DbContext
{
    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        modelBuilder.Entity<Order>(entity =>
        {
            ...
            // TASK 1: map ShippingAddress with ComplexProperty, then remove
            // NotMapped from the property on Order.

            // TASK 2: replace this broad filter with two named filters:
            // TenantFilter and ArchivedFilter.
            entity.HasQueryFilter(order => order.TenantId == "WORKSHOP" && !order.IsArchived);
        });
    }
}
...
public static class Queries
{
    ...
    {
        // TASK 1: query the order and return ShippingAddress.City.
    ...
    {
        // TASK 2: disable only ArchivedFilter by name. The OTHER tenant must stay hidden.
        return db.Orders.CountAsync();
    }

    public static Task<int> MarkReadyAsync(OrdersContext db)
    {
        // TASK 3: use ExecuteUpdateAsync with the statement-lambda form.
