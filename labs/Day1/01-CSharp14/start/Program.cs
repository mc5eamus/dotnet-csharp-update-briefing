...
Console.WriteLine("Lab 01 - C# 14 in anger");
Console.WriteLine(new string('=', 52));

// ---------------------------------------------------------------------------
// These checks must keep passing while you refactor. That is the whole point:
// the public behaviour does not change, only the amount of code behind it.
// ---------------------------------------------------------------------------
...
// TASK 3: uncomment once you have added the static extension member.
...
{
    // TASK 4: three places in this file can use null-conditional assignment.
    // This is the first. Find the other two.
    if (customer.CurrentOrder is not null)
    {
...
{
    // Note this one is a *compound* assignment. C# 14 handles that too, and the
    // right-hand side is only evaluated when the target is non-null.
    if (line is not null)
    {
        line.Quantity += extra;
    }
